...
namespace Geekwright\ComposerTool;
...
class ComposerUtility
{
     ...
     * find composer.json
     *
     * @return string path to composer.json or empty string if not found.
     */
    protected function findComposerJson()
    {
        if (!empty($this->composerJsonPath)) {
    ...
    public function composerExecute($command_line)
    {
        ...
        if (empty($this->exe)) {
            ...
            if ($foundExe) {
                $this->exe = $foundExe . ' ' . \XoopsBaseConfig::get('lib-path') . '/composer.phar';
            } else {
        ...
        $path = $this->findComposerJson();
        if (!chdir($path)) {
            $this->errors[] = 'Cannot change directory to composer.json path';
            return false;
        }
